...
Self-hosted personal-finance automation for European households. Syncs bank accounts (PSD2 open banking via [Enable Banking](https://enablebanking.com), which covers 2,500+ banks across the EEA and UK) and Interactive Brokers into a local SQLite database, categorises transactions with an editable rule engine, and renders a zero-dependency single-file HTML dashboard: spending, income, net worth, and a weekly email-style digest.
...
- **`categorize.py`** — rule-based merchant extraction & categorisation of raw bank labels (`CARTE 12/03 E.LECLERC…` → *Groceries*). The rule table is plain data — the shipped rules are tuned for French banks; edit them for your bank's label format. Household-specific patterns (e.g. internal transfers between family members) live in `config.json`, not in code.
